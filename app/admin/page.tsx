    ...
    <section className="admin-main">
      ...
      <article className="table-card payment-link-studio" id="payment-links">
        ...
        <form className="payment-link-generator" onSubmit={(event) => void generatePaymentLink(event)}>
          ...
          <button className="checkout-submit payment-link-create-button" disabled={generatingPaymentLink}><span aria-hidden="true">↗</span>{generatingPaymentLink ? "Generating secure link…" : "Generate branded payment link"}</button>
        </form>
        {generatedPaymentUrl && <div className="generated-payment-link" role="status"><div><b><span aria-hidden="true">✓</span> Payment link ready</b><a href={generatedPaymentUrl} target="_blank" rel="noreferrer">{generatedPaymentUrl}</a></div><button className="payment-link-copy-button" type="button" onClick={() => { void navigator.clipboard.writeText(generatedPaymentUrl); setNotice("Payment link copied."); }}>Copy link</button></div>}
        <div className="payment-link-history"><h3>Recent payment links</h3>{paymentLinks.map((link) => { const order = Array.isArray(link.orders) ? link.orders[0] : link.orders; return <div key={link.id}><span><b>{order?.order_number || "New link"} · {order?.customer_name}</b><small>{order?.currency} {Number(order?.total || 0).toFixed(2)} · {link.status} · expires {new Date(link.expires_at).toLocaleDateString()}</small></span>{link.status === "active" && <button className="payment-link-revoke-button" type="button" onClick={() => void revokePaymentLink(link.id)}>Revoke</button>}</div>; })}{!paymentLinks.length && <p>No payment links created yet.</p>}</div>
      </article>
