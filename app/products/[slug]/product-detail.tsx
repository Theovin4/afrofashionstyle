...
export function ProductDetail({ product, related }: { product: Product; related: Product[] }) {
  ...
  <section className="product-detail">
    ...
    <div className="product-info"><span className="eyebrow">{product.category}</span><h1>{product.name}</h1><p className="detail-price">{currency} {price.toFixed(2)}</p>
      <p className="detail-description">{product.description || `A distinctive ${product.category.toLowerCase()} design with a confident silhouette, expressive Nigerian influence and tracked delivery across the USA and UK.`}</p>
      ...
      <div className="product-promises"><p><b>Tracked delivery</b><span>Fly Logistics doorstep service</span></p><p><b>Considered preparation</b><span>Allow 5–7 working days</span></p><p><b>Fit support</b><span>Send measurements if unsure</span></p></div>
      ...
      <details><summary>Delivery &amp; order policy</summary><p>Fly Logistics provides tracked USA and UK doorstep delivery. Please allow 5–7 working days before dispatch. Outfits are final sale, except where applicable law requires a remedy for faulty or incorrectly supplied goods. Send your measurements before checkout if you are unsure of your size.</p></details>
