...
export const metadata = { title: "Privacy Policy", alternates: { canonical: "/privacy" }, robots: { index: false, follow: true } };

export default function PrivacyPage() {
  return <main><PremiumHeader/><article className="policy-page"><span className="eyebrow">Legal</span><h1>Privacy policy</h1><p>Last updated August 20, 2026.</p>
    ...
    <h2>Manual cryptocurrency payments</h2><p>If you choose cryptocurrency, we collect the selected network, amount reported, transaction reference and the payment-proof image you upload. Proof images are stored with restricted access and reviewed only to confirm or reject the associated order. Never upload a wallet password, private key, seed phrase or recovery phrase.</p>
    <h2>Advertising measurement and cookies</h2><p>Google Analytics, Google AdSense and Meta advertising storage are denied by default until you allow optional marketing cookies. If you consent, the Meta Pixel and Conversions API may receive shopping events, device identifiers, IP address, browser information and hashed contact details that you legitimately provide during checkout or a successful form submission. These signals help measure advertising performance and avoid counting the same action twice. We do not send passwords, payment-card details or private order notes to advertising platforms.</p>
